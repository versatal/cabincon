...
const StyledThursday = styled.div`
...
class Game extends Component {
  ...
  render() {
    ...
    const gameString = this.genGameString(gameDay);

    if (currentUser) {
      return (
        <React.Fragment>
        {gameSlot < 2 && <StyledThursday begin={beginCount} end={endCount}>
        {gameString}
        {currentUser.isAdmin && 
          <form className="slotAdd" onSubmit={this.addSlot.bind(this)}>
            <input type="hidden" ref="slotDay" value={gameDay}></input>
